  ...
  return (
    <div>
      <h2 id="province-heading" data-cy="ProvinceHeading">
        <Translate contentKey="jhipsterApp.province.home.title">Provinces</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="jhipsterApp.province.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/province/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="jhipsterApp.province.home.createLabel">Create new Province</Translate>
          </Link>
        </div>
      </h2>
        ...
        {provinceList && provinceList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="jhipsterApp.province.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th className="hand" onClick={sort('lng')}>
                  <Translate contentKey="jhipsterApp.province.lng">Lng</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('lng')} />
                </th>
                <th className="hand" onClick={sort('lat')}>
                  <Translate contentKey="jhipsterApp.province.lat">Lat</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('lat')} />
                </th>
            ...
            <tbody>
              {provinceList.map((province, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{province.lng}</td>
                  <td>{province.lat}</td>
                  <td>{province.createdDate ? <TextFormat type="date" value={province.createdDate} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>
                    {province.lastModifiedDate ? (
                      <TextFormat type="date" value={province.lastModifiedDate} format={APP_DATE_FORMAT} />
                    ) : null}
                  </td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/province/${province.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="jhipsterApp.province.home.notFound">No Provinces found</Translate>
            </div>
          )
        )}
      </div>
